...
export default function GetStartedPage(){
  ...
  if(done)return <main className="intake-page"><div className="intake-success"><span><Icon name="check" size={22}/></span><small>REQUEST RECEIVED</small><h1>We have what we need to review the opportunity.</h1><p>Nothing has been connected, changed, or sent yet. Recover will review the request before confirming scope, requirements, and pricing.</p><dl className="intake-receipt"><div><dt>Service requested</dt><dd>{selected.title}</dd></div><div><dt>Priorities</dt><dd>{form.needs.join(", ")}</dd></div><div><dt>Business</dt><dd>{form.businessName}</dd></div></dl><div><Link href="/" className="intake-primary">Back to website</Link><Link href="/signup" className="intake-secondary">Create a workspace</Link></div></div></main>;
