...
export default function GetStartedPage(){
  ...
  return <main className="intake-page">
    ...
    <div className="intake-layout">
      <aside><small>DONE-FOR-YOU SETUP</small><h1>Tell us what the business is losing.</h1><p>You do not need to understand APIs, workflows, providers or automation tools. Recover uses this intake to prepare the right plan and setup checklist.</p><ol><li className={step>=1?"active":""}>Business basics</li><li className={step>=2?"active":""}>What needs recovery</li><li className={step>=3?"active":""}>Choose the service</li></ol><div className="intake-trust"><Icon name="shield" size={16}/><span>No external action runs from this form. Connections and launch rules are reviewed later.</span></div></aside>
      <form onSubmit={submit}>
        ...
        <footer>{step>1?<button type="button" className="back" onClick={()=>setStep(value=>value-1)}>Back</button>:<Link href="/" className="back">Cancel</Link>}{step<3?<button type="button" className="next" onClick={()=>setStep(value=>value+1)}>Continue <Icon name="arrow" size={15}/></button>:<button className="next" disabled={busy}>{busy?"Submitting…":"Request my recovery plan"}<Icon name="arrow" size={15}/></button>}</footer>
      </form>
    </div>
  </main>;
}
